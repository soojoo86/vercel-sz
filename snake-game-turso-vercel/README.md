...
6. **积分排行榜** - 按历史最高分排名，实时更新，支持查看自己名次
7. **Turso 数据库** - 使用边缘 SQLite 数据库，全球低延迟
8. **Vercel 部署** - 一键部署到 Vercel Serverless 平台
...
- 功能：新增 / 编辑 / 删除自定义题目（4 选 1 单选，标记正确答案）
- 出题规则：优先随机抽取自定义题，不足 3 题时用内置信息安全题库补齐
- 新增的 `quiz_questions`、`daily_credits` 表会在首次访问相关 API 时自动创建，**Vercel 上无需手动跑初始化脚本**
